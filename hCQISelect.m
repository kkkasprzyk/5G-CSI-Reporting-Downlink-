...
function [l2sm,cqiIndex,effectiveSINR,transportBLER] = cqiSelect(l2sm,carrier,pdsch,xOverhead,SINRs,cqiTableName)
    ...
    persistent cqiTabArray tableInput;
    if (isempty(cqiTabArray)||~strcmpi(cqiTableName,tableInput))
        tableInput = cqiTableName;
        cqiTableClass = nrCQITables;
        cqiTabArray = [cqiTableClass.(['CQI' cqiTableName]).Qm (cqiTableClass.(['CQI' cqiTableName]).TargetCodeRate)*1024];
    end
    ...
    [l2sm,cqiIndex,cqiInfo] = nr5g.internal.L2SM.cqiSelect(l2sm,carrier,pdsch,xOverhead,SINRs,cqiTabArray,blerThreshold);
